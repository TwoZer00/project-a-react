...
export function Player({ post, playerDisplay, playerAction }) {
  ...
  const handleProgressClick = (e) => {
    ...
    audioElement.current.currentTime = porcentageToSecs(
      pixToPorcentage(Math.round(x), e.target.clientWidth),
    ...
    console.log(
      ...
      porcentageToSecs(
        pixToPorcentage(x, e.target.clientWidth),
        audioElement.current.duration
      ).toFixed(2)
    );
  };
          ...
          <div className="flex flex-row gap-x-2 gap-y-1 items-center h-full">
            ...
            <div className="bg-white dark:bg-neutral-900 dark:text-white flex-auto w-4/6 lg:w-5/6 flex flex-row items-center gap-x-1 pr-2">
              ...
              >
                <PlayButton
                  state={audioState}
                  className="dark:fill-purple-200 fill-purple-700"
                />
              </div>
              <div className="flex flex-col flex-auto">
                <div className="flex-auto w-full flex">
                  ...
                  <p className="bg-purple-700 text-white rounded px-1 h-fit">
                    {stringToDate(post.date.seconds)}
                  </p>
                  <ChevronDownIcon
                ...
                <div className="flex flex-row gap-2 items-center">
                  <div className="flex-1 w-full flex items-center gap-x-1">
                    {post.nsfw ? (
                      <p className="h-fit w-fit font-bold text-red-600 outline outline-2 outline-red-700 rounded px-1 text-xs">
                        NSFW
                      </p>
                    ) : (
                      ""
                ...
                <div className="flex-auto w-full">
                  ...
                  <div className="w-full h-8 relative">
                    <p className="absolute bottom-0 left-0 text-sm select-none pointer-events-none">
                      {audioProgressTime.toString().toHHMMSS() || "00:00"}
                    </p>
                    <div
                      className="w-full h-3 bg-purple-200 rounded-full overflow-hidden absolute inset-0 hover:cursor-pointer"
                      onClick={handleProgressClick}
                    >
                      <div
                        className={`transition-all pointer-events-none bg-purple-700 drop-shadow-lg relative h-full after:h-full after:w-3 after:bg-purple-100 after:rounded-full after:border-2 after:absolute after:border-purple-500 after:-right-2 after:inset-y-0 after:right-0 rounded-r-lg`}
                        style={{
                          width: `${Math.round(
                            secToPorcentage(
                              audioProgressTime,
                    ...
                    <p className="absolute bottom-0 right-0 text-sm select-none pointer-events-none">
                      {audioElement?.current?.duration > 0
                        ? audioElement?.current?.duration?.toString().toHHMMSS()
                        : "00:00"}
                    </p>
                  </div>
                </div>
