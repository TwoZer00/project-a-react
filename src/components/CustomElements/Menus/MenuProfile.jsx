...
export function MenuProfile({ user, profileImage, isDown, setDark, ...props }) {
  ...
  return (
    <>
      <div
        {...props}
        className={`w-72 h-fit mt-2 bg-white outline outline-white outline-1 dark:border-white rounded absolute top-full right-0 drop-shadow-md dark:bg-neutral-900 cursor-auto ${isDown ? "" : "hidden"
          }`}
      >
        <div className="flex flex-row w-full justify-center items-center gap-x-2 h-28">
          <div className="rounded-full h-full overflow-hidden h-20 w-20">
            <img
              ...
              alt=""
              className={`w-full h-full ${profileImage ? "object-cover" : "p-2.5 bg-purple-700"
                }`}
            />
          </div>
        ...
        <div className="flex flex-col items-center divide-y">
          ...
          <div className="w-full hover:bg-purple-200/20 hover:cursor-pointer">
            <a className="py-2 dark:text-white text-center w-full block" href="https://dashboard.aproject.twozer00.dev">Dashboard</a>
          </div>
        </div>
      </div>
    </>
